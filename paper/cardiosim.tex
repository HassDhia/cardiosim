...
PPO agents substantially outperform random baselines on pacing control and antiarrhythmic dosing (5.1$\times$ improvement on dosing). On the defibrillation environment, PPO achieves comparable performance to random (ratio 0.95), revealing that the stochastic nature of defibrillation success makes uninformed high-frequency shocking a surprisingly competitive strategy. The pacing control environment shows the strongest PPO advantage, consistent with its smooth reward landscape.

The antiarrhythmic dosing environment presents the greatest pharmacokinetic challenge due to delayed drug effects and the narrow therapeutic window. The defibrillation timing environment presents a distinct challenge: sparse, event-driven rewards and stochastic success probabilities create a setting where exploration-driven random policies benefit from frequent shocking, while PPO must learn when to withhold and when to deliver shocks optimally.
