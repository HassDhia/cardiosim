...
\begin{align}
    ...
    \frac{dv}{dt} &= \left(\epsilon_0 + \frac{\mu_1 v}{\mu_2 + u}\right)(-v - ku(u - a - 1))
\end{align}
...
The single-compartment PK model describes drug concentration dynamics with first-order absorption:
\begin{align}
    \frac{dA_{\text{gut}}}{dt} &= -k_a A_{\text{gut}} + D \\
    \frac{dC}{dt} &= \frac{k_a A_{\text{gut}}}{V_d} - k_e C
\end{align}
where $A_{\text{gut}}$ is the gut compartment amount, $C$ is plasma concentration, $D$ is the administered dose, $V_d$ is the volume of distribution, $k_a$ is the absorption rate constant, and $k_e$ is the elimination rate constant. Pharmacodynamic effects follow a sigmoid $E_{\max}$ model:
\begin{equation}
...
\textbf{Dynamics:} The FHN model simulates cardiac dynamics with drug-modulated parameters. A single-compartment PK model tracks drug concentration. The PD model modulates action potential parameters based on drug effect.
...
\textbf{Dynamics:} The Aliev-Panfilov model operates in a chaotic regime simulating fibrillation. Shock delivery resets the membrane state probabilistically, with success probability increasing with energy and depending on the fibrillation phase.
...
CardioSim is implemented in Python with NumPy for numerical integration (custom fourth-order Runge-Kutta). The package follows a modular architecture:
